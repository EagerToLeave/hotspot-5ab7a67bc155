...
void ObjectMonitor::post_monitor_wait_event(EventJavaMonitorWait *event,
    ...
    event->set_address((TYPE_ADDRESS)
    this->object_addr());
...
void ObjectMonitor::sanity_checks() {
    int error_cnt = 0;
    int warning_cnt = 0;
    bool verbose = Knob_Verbose != 0
    NOT_PRODUCT( || VerboseInternalVMTests);

    if (verbose) {
        tty->print_cr("INFO: sizeof(ObjectMonitor)="
        SIZE_FORMAT,
                sizeof(ObjectMonitor));
        tty->print_cr("INFO: sizeof(PaddedEnd<ObjectMonitor>)="
        SIZE_FORMAT,
